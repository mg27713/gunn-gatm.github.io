...
export class Props {
   ...
   * @returns {boolean}
   */
  isPropertyInheritable (propName) { // not doing inheritance for now...
...
export class VisComponent {
  ...
  getClasses () {
    return this.domElement ? Array.from(this.domElement.classList) : []
  }

  addClass (name) {
    if (this.domElement) this.domElement.classList.add(name)
  }

  removeClass (name) {
    if (this.domElement) this.domElement.classList.remove(name)
  }
...
export class VisGroup extends VisComponent {
  constructor() {
    super()
...
export class SnapVisualization extends VisGroup {
  constructor() {
    super()
  }
